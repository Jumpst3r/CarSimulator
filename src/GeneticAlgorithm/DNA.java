...
public class DNA {
    ...
    private double calculateFitness(DNA dna) {
        ...
        new Thread(() -> {
            ...
            long stop = System.nanoTime() + TimeUnit.MILLISECONDS.toNanos(500);
            while (stop > System.nanoTime()){
                error = sp - simulator.getSpeed();
                integral = integral + (error * dt);
                absErr.setAbsErr(Math.abs(absErr.getAbsErr() + (error * dt)));
    ...
    public DNA crossOver(DNA genA) {
        ...
        double n_KD = (genA.genes[1] + this.genes[1]) / 2;
        double n_KI = (genA.genes[2] + this.genes[2]) / 2;
    ...
    private DNA mutate(DNA child) {
        ...
        if (rnd.nextDouble() < GeneticAlgorithm.MUTATION_RATE) {
            int i = rnd.nextInt(3);
            child.genes[i] = 5.0 * rnd.nextDouble();
